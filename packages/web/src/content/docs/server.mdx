...
| `-p, --password`   | Basic auth password (defaults to `OPENCODE_SERVER_PASSWORD` or the running server's) | generated when starting a server |
...
- `host` binds to all interfaces (`0.0.0.0`) by default so remote clients can reach it — pass `--hostname 127.0.0.1` to restrict it to the local machine. Password auth is always enabled.
...
`host` publishes the server through [Tailscale Serve](https://tailscale.com/kb/1242/tailscale-serve) when the `tailscale` CLI is available, so the phone pairs over real HTTPS (`https://<machine>.ts.net`) instead of plain HTTP — the QR code points at the HTTPS URL when it is available. The tunnel is tailnet-only, end-to-end encrypted, and the serve config persists at the tailnet level (re-running `host` re-applies it idempotently). Pass `--no-qr` to skip pairing output entirely.
