...
- After starting a server, `opencode host` waits for you to press Enter before launching the TUI, so you have time to scan the QR code. The server stays alive while you pair; the idle timer below only starts once clients have connected.
