...
When called without arguments, attach first tries `/tmp/opencode.sock`. A socket file on disk does not guarantee a live listener — a crashed server leaves it behind — so the socket is probe-connected first. If it refuses connections, a warning is printed and attach falls back to `http://127.0.0.1:8090`. Pass a `url` or `--socket` explicitly to override discovery.
...
Start (or reuse) an opencode server, print a QR code for pairing the PWA / Web UI, and attach a TUI to that server — all in one command. Further `opencode host` invocations in other terminals or directories attach to the same server and share its sessions.

```bash
opencode host [project]
```
...
# Other terminals / tmux windows: attach to the same server from other directories
opencode host ~/work/project-b
```

Both flows — starting a server and attaching to one that is already running — print the QR code and wait for you to press Enter before opening the TUI (it auto-starts after 15 seconds), so you have time to scan the code.

See [Host a shared server](/docs/server#host-a-shared-server) for options and lifecycle details.
