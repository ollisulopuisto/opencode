...
Execution model:
OpenCode Go model pool (dynamically selected)

Supervisor:
Agy / Gemini (strategic supervisory layer)
...
The system separates execution models from the supervisory layer. Multiple execution models may coexist; supervision is invoked selectively.

## Model pool = execution capacity

No individual model is a permanent architectural dependency. OpenCode Go provides a changing pool of models with different capability, cost, latency, and reliability characteristics. The harness selects the best currently available model for each work unit.

Qwen3.8 Max is a first-class benchmark candidate, not a required dependency. It may disappear or change availability without invalidating the harness.

Execution models should perform:
...
Compatible execution models may be run concurrently when tasks are demonstrably independent and the environment supports safe concurrency.
...
The implementer is selected by the model router. Qwen3.8 Max is one candidate, not a required implementation model.
...
# 23. Model Adaptation Layer

The harness must adapt to the selected model rather than forcing every model to imitate one canonical model.
...
Do not hard-code additional model dependencies. New models may be added to the pool without architectural changes; routing decisions must be benchmark-driven.

# 27A. Model Registry and Adaptive Routing

The harness must treat model availability as dynamic.

The model registry records, for each currently available model:
...
Model identity is configuration, not architecture.

The router selects models by capability and task requirements, not by hard-coded
model names.

Example routing concepts:

```yaml
routing:
  explorer:
    preference: high_throughput
  implementer:
    preference: coding_capability
  debugger:
    preference: reasoning_plus_coding
  verifier:
    preference: independent_verification
  architect:
    preference: highest_reasoning_available
```
...
### Model-loss requirement

If a preferred model becomes unavailable during a task:

1. detect the failure
2. refresh model availability
3. preserve task state
4. select the best compatible fallback
5. continue from the current state
6. verify the result
7. record the substitution

No core state, prompt, workflow, or orchestration component may require a
specific model to exist.

### Model adaptation

Model-specific behavior belongs behind an adapter boundary.
...
The orchestration state machine must remain model-independent.
...
## Phase 6 — Model Routing and Model Adaptation
...
- benchmark-driven routing metrics

No model becomes a permanent dependency.

## Phase 7 — Gemini Supervisor
...
## Phase 8 — Model-Pool Optimization
...
# 33. Model Churn and Continuity

Model availability is expected to change. The harness must remain operational when a preferred model is removed, renamed, rate-limited, or temporarily unavailable.

Do not encode model-specific assumptions in persistent task state.

Record model substitutions as observable execution events.
...
Make the selected model smarter.
...
Use the model pool for breadth and execution.
Use Gemini for high-value judgment.

The harness should maximize useful work per unit of supervisory intelligence.


# 38. Model Independence Principle

Models are disposable resources. Harness architecture is persistent infrastructure.

The harness must be able to replace its preferred execution model without changing
its state machine, verification semantics, recovery semantics, permissions,
or orchestration architecture.

Qwen3.8 Max is therefore treated as a valuable experimental worker and benchmark
subject, not as an architectural foundation.
